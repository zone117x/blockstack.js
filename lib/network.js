...
class BlockstackNetwork {
    constructor(apiUrl, broadcastServiceUrl, bitcoinAPI, network = bitcoinjs_lib_1.default.networks.bitcoin) {
    ...
    coerceAddress(address) {
        const { hash, version } = bitcoinjs_lib_1.default.address.fromBase58Check(address);
        const scriptHashes = [bitcoinjs_lib_1.default.networks.bitcoin.scriptHash,
            bitcoinjs_lib_1.default.networks.testnet.scriptHash];
        const pubKeyHashes = [bitcoinjs_lib_1.default.networks.bitcoin.pubKeyHash,
    ...
    getZonefile(zonefileHash) {
        return fetch(`${this.blockstackAPIUrl}/v1/zonefiles/${zonefileHash}`)
            ...
            if (resp.status === 200) {
                return resp.text()
                    .then((body) => {
                    const sha256 = bitcoinjs_lib_1.default.crypto.sha256(Buffer.from(body));
    ...
    */
    ...
    modifyUTXOSetFrom(txHex) {
        const tx = bitcoinjs_lib_1.default.Transaction.fromHex(txHex);
        ...
        tx.outs.forEach((utxoCreated, txOutputN) => {
            ...
            const address = bitcoinjs_lib_1.default.address.fromOutputScript(utxoCreated.script, this.layer1);
            let includeSet = [];
            if (this.includeUtxoMap.hasOwnProperty(address)) {
...
class LocalRegtest extends BlockstackNetwork {
    constructor(apiUrl, broadcastServiceUrl, bitcoinAPI) {
        super(apiUrl, broadcastServiceUrl, bitcoinAPI, bitcoinjs_lib_1.default.networks.testnet);
    }
    getFeeRate() {
        ...
        return fetch(`${this.utxoProviderUrl}/pushtx?cors=true`, {
            ...
            return text
                ...
                if (respText.toLowerCase().indexOf('transaction submitted') >= 0) {
                    const txHash = Buffer.from(bitcoinjs_lib_1.default.Transaction.fromHex(transaction)
                        .getHash()
